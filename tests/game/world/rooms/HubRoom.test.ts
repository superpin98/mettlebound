...
function makeRoom(id = 'hub_01'): HubRoom {
  const scene: Scene        = { materials: { forEach: vi.fn() }, markAllMaterialsAsDirty: vi.fn() } as unknown as Scene;
  const assetManager: AssetManager = { loadAsset: vi.fn().mockResolvedValue({}), instantiate: vi.fn().mockReturnValue({ rootNode: { parent: null, position: { x: 0, y: 0, z: 0 }, rotation: { x: 0, y: 0, z: 0 } }, dispose: vi.fn() }) } as unknown as AssetManager;
  return new HubRoom({ id, scene, assetManager });
}
...
describe('HubRoom', () => {
  ...
  describe('dispose()', () => {
    ...
    it('dispose() llama dispose del rootNode', async () => {
      room.dispose();
