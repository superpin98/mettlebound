...
function makeRoom(id = 'combat_01'): CombatTriggerRoom {
  return new CombatTriggerRoom({ id, scene: { materials: { forEach: vi.fn() }, markAllMaterialsAsDirty: vi.fn() } as unknown as Scene, assetManager: { loadAsset: vi.fn().mockResolvedValue({}), instantiate: vi.fn().mockReturnValue({ rootNode: { parent: null, position: { x: 0, y: 0, z: 0 }, rotation: { x: 0, y: 0, z: 0 } }, dispose: vi.fn() }) } as unknown as AssetManager });
}

describe('CombatTriggerRoom', () => {
  let room: CombatTriggerRoom;
  ...
  describe('identidad', () => {
    ...
    it('shape es L_shape', () => {
      expect(room.shape).toBe('L_shape');
    });
  ...
  describe('build()', () => {
    ...
    it('getLightSources() tiene 3 luces tras build()', async () => {
      await room.build();
      expect(room.getLightSources()).toHaveLength(3);
    });

    it('build() dos veces es no-op (3 luces, no 6)', async () => {
      await room.build();
      await room.build();
      expect(room.getLightSources()).toHaveLength(3);
    });
